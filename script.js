 ...
 * ---------------------------------
 * V2 UI (per approved wireframe): 4 buttons on the right rail --
 * Read, Show, Export (dummy for now), Clear.
 *
 * Status colors on the camera overlay:
 ...
 *   Show   -> full-screen list of everything recorded so far;
 *             tap ANYWHERE on the panel to close it
 *   Export -> DUMMY for now, does nothing (placeholder per request)
...
const recordedNumInPanelEl = document.getElementById('recordedNumInPanel');
...
function updateRecordedCount() {
  ...
  recordedNumInPanelEl.textContent = String(recorded.size);
}
...
function onExportDummy() {
  // Placeholder on purpose -- not wired up to real export logic yet.
}
...
function renderShowPanel() {
  ...
  showList.innerHTML = '';
  if (rows.length === 0) {
    showEmpty.classList.remove('hidden');
    return;
  }
  showEmpty.classList.add('hidden');
...
function openShow() {
  renderShowPanel();
  showPanel.classList.add('visible');
}

function closeShow() {
  showPanel.classList.remove('visible');
}

btnRead.addEventListener('click', onRead);
btnShow.addEventListener('click', openShow);
btnExport.addEventListener('click', onExportDummy);
btnClear.addEventListener('click', onClear);
// Tap ANYWHERE on the show panel closes it, per the approved wireframe.
showPanel.addEventListener('click', closeShow);
